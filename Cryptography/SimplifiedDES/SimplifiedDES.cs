...
public static class SimplifiedDES
{
    ...
    private static void Mix(ref BitArray bitArray, int[] mixTemplate)
    {
        ...
        for (var i = 0; i < newBitArray.Length; i++) newBitArray[i] = bitArray[mixTemplate[i] - 1];
    ...
    private static void CyclicLeftShift(BitArray bitArray, int bitsAmount)
    {
        ...
        for (var k = i; k < i + bitsAmount; k++) bitsToShift[counter++] = bitArray[k];

        while (i < bitArray.Length / 2 - bitsAmount) bitArray[i] = bitArray[i++ + bitsAmount];

        counter = 0;

        for (var k = bitArray.Length / 2 - bitsAmount; k < bitArray.Length / 2; k++)
        ...
        while (i < bitArray.Length - bitsAmount) bitArray[i] = bitArray[i++ + bitsAmount];

        counter = 0;

        for (var k = bitArray.Length - bitsAmount; k < bitArray.Length; k++) bitArray[k] = bitsToShift[counter++];
    }
    ...
    private static void Swap(BitArray bitArray)
    {
        ...
        for (var i = 0; i < bitArray.Length / 2; i++) bitArray[i] = boolArray[counter++];
    }
    ...
    private static int BitArrayToInt32(BitArray bitArray)
    {
        ...
        for (var i = bitArray.Length - 1; i >= 0; i--) result += (bitArray[i] ? 1 : 0) * (int)Math.Pow(2, digit++);
        return result;
    }
    ...
    private static void Round(BitArray input, BitArray key)
    {
        ...
        for (var i = 0; i < sequence.Length; i++) input[i] = sequence[i];
    }
    ...
    private static byte[] Chipher(byte[] text, BitArray key, bool encrypt)
    {
        ...
        {
            var bitArray = new BitArray(new[] { text[i] });
